...
#include "src/operators/materialized_scan.h"
...
static void CsvToColumnar(const std::string &schemaPath,
    ...
    const Schema schema = Schema::LoadFromCSV(schemaPath);
    std::cerr << "Loaded schema: " << schema.NumColumns() << " columns\n";

    const CsvReader reader(dataPath);
    ...
    while (true) {
        ...
        for (auto &cd: schema.columns) {
            rg.columns.push_back(
                ColumnPtr(Column::Create(cd.typeId).release()));
        }
        ...
        if (rg.numRows > 0) {
            for (const auto &col: rg.columns) {
                while (col->Size() < rg.numRows) {
                    if (col->Type().isFixedSize()) {
                        col->PushFromString("0");
                    } else {
                        col->PushFromString("");
                    }
                }
...
void WriteBatchToCSV(const Batch &batch, const std::string &csv_path) {
    const CsvWriter writer(csv_path);
    ...
    for (size_t r = 0; r < batch.numRows; ++r) {
        ...
        for (const auto &column: batch.columns) {
            row.push_back(column->GetAsString(r));
        }
        writer.WriteRow(row);
    }
}
...
int main(int argc, char **argv) {
    if (argc >= 2 && std::string(argv[1]) == "convert") {
        if (argc < 4) {
            std::cerr << "Usage: ColumnarEngine convert <input.csv> <output.col>\n";
            return 1;
        }
        std::string inputCsv = argv[2];
        std::string outputCol = argv[3];
        std::string schemaPath = (argc >= 5) ? argv[4] : "scheme.csv";

        CsvToColumnar(schemaPath, inputCsv, outputCol);
        return 0;
    }

    if (argc >= 2 && std::string(argv[1]) == "query") {
        if (argc < 5) {
            std::cerr << "Usage: ColumnarEngine query <N> <input.col> <output.csv>\n";
            return 1;
        }
        int queryNum = std::stoi(argv[2]);
        std::string colPath = argv[3];
        std::string outputPath = argv[4];

        RunQuery(queryNum, colPath, outputPath);
        return 0;
    }

    ColumnarReader reader("hits.col");
    ...
    auto colType = [&](int colIdx) -> TypesId {
        return schema.columns[colIdx].typeId;
    };
    ...
    // 5. SELECT COUNT(DISTINCT UserID) FROM hits;
    {
        ...
        auto scan = std::make_unique<TableScan>(reader, std::vector<size_t>{size_t(user_id)});
        std::vector<GlobalAggregate::Spec> specs;
        specs.emplace_back(GlobalAggregate::AggOp::COUNT_DISTINCT, std::make_unique<ColumnRef>(0, colType(user_id)));
        auto agg = std::make_unique<GlobalAggregate>(std::move(scan), std::move(specs));
        auto res = agg->Next();
    ...
    // 6. SELECT COUNT(DISTINCT SearchPhrase) FROM hits;
    {
        ...
        std::vector<GlobalAggregate::Spec> specs;
        specs.emplace_back(GlobalAggregate::AggOp::COUNT_DISTINCT, std::make_unique<ColumnRef>(0, colType(search_phr)));
        auto agg = std::make_unique<GlobalAggregate>(std::move(scan), std::move(specs));
        auto res = agg->Next();
    ...
    // 9. SELECT RegionID, COUNT(DISTINCT UserID) AS u FROM hits GROUP BY RegionID ORDER BY u DESC LIMIT 10;
    {
        ...
        std::vector<GroupAggregate::Spec> agg_specs;
        agg_specs.emplace_back(GroupAggregate::AggOp::COUNT_DISTINCT, std::make_unique<ColumnRef>(1, colType(user_id)));
    ...
    // 10. SELECT RegionID, SUM(AdvEngineID), COUNT(*) AS c, AVG(ResolutionWidth), COUNT(DISTINCT UserID) FROM hits GROUP BY RegionID ORDER BY c DESC LIMIT 10;
    {
        ...
        agg_specs.emplace_back(GroupAggregate::AggOp::COUNT_DISTINCT, std::make_unique<ColumnRef>(3, colType(user_id)));
        auto group = std::make_unique<GroupAggregate>(std::move(scan), std::move(keys), std::move(agg_specs));
        std::vector<TopK::SortKey> top_keys;
    ...
    // 12. SELECT MobilePhone, MobilePhoneModel, COUNT(DISTINCT UserID) AS u FROM hits WHERE MobilePhoneModel <> '' GROUP BY MobilePhone, MobilePhoneModel ORDER BY u DESC LIMIT 10;
    {
        ...
        std::vector<GroupAggregate::Spec> agg_specs;
        agg_specs.emplace_back(GroupAggregate::AggOp::COUNT_DISTINCT, std::make_unique<ColumnRef>(2, colType(user_id)));
        auto group = std::make_unique<GroupAggregate>(std::move(filter), std::move(keys), std::move(agg_specs));
        std::vector<TopK::SortKey> top_keys;
    ...
    // 15. SELECT SearchEngineID, SearchPhrase, COUNT(*) AS c FROM hits WHERE SearchPhrase <> '' GROUP BY SearchEngineID, SearchPhrase ORDER BY c DESC LIMIT 10;
    {
        ...
        auto scan = std::make_unique<TableScan>(reader, std::vector<size_t>{size_t(search_eng), size_t(search_phr)});
        auto pred = std::make_unique<Comparison>(CmpOp::NE,
    ...
    // 27. SELECT SearchPhrase FROM hits WHERE SearchPhrase <> '' ORDER BY EventTime, SearchPhrase LIMIT 10;
    {
        ...
        auto scan = std::make_unique<TableScan>(reader, std::vector<size_t>{size_t(search_phr), size_t(event_time)});
        auto pred = std::make_unique<Comparison>(CmpOp::NE,
                                                 std::make_unique<ColumnRef>(0, colType(search_phr)),
                                                 std::make_unique<Constant>(std::string("")));
        auto filter = std::make_unique<Filter>(std::move(scan), std::move(pred));
    ...
    // bool success = CompareQueryResults("D:/ColumnarEngine/cmake-build-debug/small/small", ".");
    // // текущая папка с q*.csv
    // if (success) std::cout << "All queries match!" << std::endl;
    // else std::cerr << "Mismatches found." << std::endl;
    return 0;
}
